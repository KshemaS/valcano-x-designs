  ...
  const steps = [
    {
      ...
      desc: 'We carefully understand your business goals, audience, and project scope.',
    ...
    },
    {
      ...
      desc: 'Our team creates modern, strategic, and user-focused design concepts.',
      icon: <Coffee size={32} />,
    ...
    },
    {
      id: 'step-3',
      title: 'Revisions',
      desc: 'We refine and improve based on your feedback until everything is perfect.',
    ...
    },
    {
      ...
      desc: 'We deliver high-quality files or fully functional digital solutions on schedule.',
            ...
            {steps.map((step, index) => {
              ...
              return (
                <div
                  ...
                  className={`relative group cursor-pointer border-l-4 pl-10 py-6 transition-all duration-500 ${isActive ? 'border-primary bg-white/[0.02] rounded-r-2xl' : 'border-white/10 hover:border-white/30 hover:bg-white/[0.01] rounded-r-2xl'
                    }`}
                >
                  {/* Subtle Background Number */}
                  <span className={`absolute right-6 top-1/2 -translate-y-1/2 text-8xl font-black font-everett pointer-events-none transition-all duration-500 ${isActive ? 'text-white/5 tracking-tighter' : 'text-transparent'
                    }`}>
                    0{index + 1}
                  </span>

                  <h4 className={`text-4xl font-bold font-everett mb-4 transition-colors duration-500 relative z-10 ${isActive ? 'text-primary' : 'text-white group-hover:text-white/80'
                    }`}>
                    {step.title}
                  </h4>

                  {/* Accordion Description */}
                  <div className={`grid transition-all duration-500 ease-in-out relative z-10 ${isActive ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'
                    }`}>
